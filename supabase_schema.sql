...
BEGIN
    ...
    -- 2. Loop through all active students in this grade stream
    FOR student_record IN 
        SELECT id FROM students 
        WHERE current_grade_stream_id = p_grade_stream_id AND status = 'ACTIVE'
    LOOP
        -- 3. Upsert the master report_cards record
        INSERT INTO report_cards (student_id, academic_year_id, term_id, grade_stream_id, generated_by_user_id)
        VALUES (student_record.id, p_academic_year_id, p_term_id, p_grade_stream_id, auth.uid())
        ...
        -- 4. Calculate per-subject aggregates for this student
        -- Delete old subjects if re-generating
        DELETE FROM report_card_subjects WHERE report_card_id = v_report_card_id;
        ...
        WHERE em.student_id = student_record.id
          AND e.academic_year_id = p_academic_year_id
          AND e.term_id = p_term_id
        GROUP BY e.subject_id;

        -- We could also trigger a grade_symbol calculation here natively matching the earlier trigger logic
    END LOOP;
END;
