    ...
    <ModalOverlay onClose={onClose}>
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><label className="block text-xs text-[var(--color-text-muted)] mb-1">First Name *</label><input className="input-field w-full" value={props.formFirstName} onChange={e => props.setFormFirstName(e.target.value)} required /></div>
          <div><label className="block text-xs text-[var(--color-text-muted)] mb-1">Last Name *</label><input className="input-field w-full" value={props.formLastName} onChange={e => props.setFormLastName(e.target.value)} required /></div>
        </div>

        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><label className="block text-xs text-[var(--color-text-muted)] mb-1">Phone Number *</label><input className="input-field w-full" type="tel" value={props.formPhone} onChange={e => props.setFormPhone(e.target.value)} placeholder="e.g. 0712345678" required /></div>
          <div>
        ...
        {props.formRole === 'STUDENT' && (
          <div className="border-t border-[var(--color-border)] pt-4 mt-4">
            ...
            <div className="mb-4"><label className="block text-xs text-[var(--color-text-muted)] mb-1">Admission Number *</label><input className="input-field w-full" value={props.formAdmissionNumber} onChange={e => props.setFormAdmissionNumber(e.target.value)} placeholder="e.g. ADM-001" required /></div>
        ...
        {props.formRole === 'CLASS_TEACHER' && (
          <div className="border-t border-[var(--color-border)] pt-4 mt-4">
            ...
            <div><label className="block text-xs text-[var(--color-text-muted)] mb-1">Class</label>
