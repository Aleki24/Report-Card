...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!grade_stream_id || !academic_level_id) {
            return NextResponse.json({ error: 'Grade stream and academic level are required.' }, { status: 400 });
        }
        if (!admin_user_id) {
        ...
        if (existingStudent) {
            return NextResponse.json({ error: `A student with admission number "${admission_number.trim()}" already exists.` }, { status: 409 });
        }

        // 1. Generate a UUID for the new student
        const userId = crypto.randomUUID();
        const placeholderEmail = `${admission_number.trim().toLowerCase().replace(/[^a-z0-9]/g, '')}@student.local`;
