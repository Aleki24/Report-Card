...
export async function seedExamSlots(options: SeedOptions) {
  ...
  for (const gradeId of gradeIds) {
    ...
    for (const subject of gradeSubjects) {
      // Skip subjects taught in a different band of the same curriculum —
      // an Upper Primary learning area has no business becoming a Grade 11
      // exam slot. An explicit teacher assignment overrides the check, since
      // the school has said outright that this class takes this subject.
      const assigned = assignedSubjectGrades.has(`${subject.id}|${gradeId}`);
      if (!assigned && !isSubjectOfferedAtGrade(subject, grade)) {
        continue;
      }
